...
// is sent to hub.infomentor.se too) and host-only cookies (sent only
// to the exact host that set them).
interface Cookie {
	name: string;
	value: string;
	domain: string;
...
export function createCookieJar(): CookieJar {
	...
	return {
		applyResponse(response, requestUrl) {
			...
			// Node 20+ exposes getSetCookie; older runtimes fell back to
			// parsing the combined header. We only support Node 22+ here.
