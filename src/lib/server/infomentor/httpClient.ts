...
export function createSession(): Session {
	 ...
	 * jar updated at every hop. Mirrors browser-compatible (not strictly
	 * spec-compliant) redirect behaviour: 303 always becomes GET, 301/302
	 * downgrade a POST to GET (what curl -L and every browser actually
	...
	async function request(
		...
		for (;;) {
			...
			// status 0 / type "opaqueredirect" happens if something
			// upstream still intercepts it; treat as a dead end rather
